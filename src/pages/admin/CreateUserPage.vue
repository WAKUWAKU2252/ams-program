...
import { listDepartments, listEmployees } from '@/shared/services/master.service'
import type { DepartmentOption, EmployeeOption } from '@/shared/services/master.service'
...
<template>
  <!-- ปรับเป็น flex justify-center เพื่อให้อยู่ตรงกลางจอ (หากต้องการใช้ grid แบบเดิมสามารถเปลี่ยนกลับได้) -->
  <div class="flex justify-left p-6">
    
    <!-- ปรับ max-w-2xl เป็น max-w-4xl เพื่อให้พอดีกับฟอร์ม 2 คอลัมน์ -->
    <div class="card w-full max-w-4xl bg-base-100 shadow-xl">
      <form class="card-body" @submit.prevent="onSubmit">
        
        <div class="grid md:grid-cols-2 gap-8">
          
          <!-- ==================== ส่วนที่ 1: บัญชีผู้ใช้ ==================== -->
          <div>
            <h1 class="card-title text-2xl">Create User</h1>
            ...
            <fieldset class="fieldset">
              ...
              <legend class="fieldset-legend mt-2">Display name</legend>
              ...
              <p class="label-text-alt mt-1 text-base-content/60">
                ชื่อบัญชีที่ผู้ใช้ตั้งเอง - ชื่อที่โชว์ในเอกสาร/อีเมลใช้ชื่อพนักงานแทน
              </p>

              <legend class="fieldset-legend mt-2">Password</legend>
              <input v-model="form.password" type="password" class="input w-full" placeholder="Password" minlength="4" required />

              <legend class="fieldset-legend mt-2">Role</legend>
          ...
          <!-- ==================== ส่วนที่ 2: ข้อมูลพนักงาน ==================== -->
          <div>
            <!-- โหมด: สร้างใหม่ / ผูกคนเดิม / ไม่ผูก -->
            <div role="tablist" class="tabs tabs-box tabs-sm mb-4">
              <button type="button" role="tab" class="tab" :class="{ 'tab-active': mode === 'new' }" @click="mode = 'new'">
                สร้างใหม่
              </button>
              <button type="button" role="tab" class="tab" :class="{ 'tab-active': mode === 'link' }" @click="mode = 'link'">
                ผูกคนเดิม
              </button>
              <button type="button" role="tab" class="tab" :class="{ 'tab-active': mode === 'none' }" @click="mode = 'none'">
                ไม่ผูก
              </button>
            </div>
            ...
            <fieldset v-if="mode === 'new'" class="fieldset">
              <div class="grid gap-x-4 gap-y-2 sm:grid-cols-2">
                ...
                <div>
                  ...
                  <p class="label-text-alt mt-1 text-base-content/60">ตัวอักษรผสมได้ - ห้ามซ้ำ</p>
                </div>
                <div>
                  ...
                  <p class="label-text-alt mt-1 text-base-content/60">ห้ามซ้ำ</p>
                </div>
              </div>
            ...
            <fieldset v-else-if="mode === 'link'" class="fieldset">
              ...
              <div class="join w-full">
                <input v-model="employeeSearch" class="input join-item w-full" placeholder="พิมพ์ชื่อหรือรหัสพนักงาน" @keyup.enter="searchEmployees" />
              ...
              <p v-else-if="employeeSearch && !searching" class="label-text-alt mt-2 text-error">
                ไม่พบพนักงานที่ตรงกับคำค้น
              </p>
            </fieldset>

            <!-- โหมด none -->
            <div v-else role="alert" class="alert alert-warning alert-soft mt-2">
            ...
            <!-- แจ้งเตือนผลลัพธ์ (Error / Success) -->
            <div v-if="errorMsg" role="alert" class="alert alert-error alert-soft mt-4">
              <span>{{ errorMsg }}</span>
            </div>
            <div v-if="successMsg" role="alert" class="alert alert-success alert-soft mt-4">
